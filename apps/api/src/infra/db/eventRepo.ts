  ...
  return {
    async addEvents(events, userId) {
      ...
      return withAuthClient(userId, async (client) => {
        ...
        const query = `
          INSERT INTO events (id, type, payload, store_id, created_at, user_id)
          VALUES ${placeholders}
        ...
        if (storeUpdates.size > 0) {
          ...
          `;
          await client.query(storeQuery, storeValues);
        }
        ...
        if (foundEvents.length > 0) {
          ...
          `;
          await client.query(upsertQuery, foundValues);
        }
        ...
        if (notFoundEvents.length > 0) {
          ...
          const notFoundQuery = `
            ...
            DO UPDATE SET
              ...
              updated_at = EXCLUDED.updated_at
          `;
          await client.query(notFoundQuery, notFoundValues);
        }
