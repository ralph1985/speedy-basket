...
export function createPackRepository(): PackRepository {
  return {
    async getPackDelta(storeId, since) {
      ...
      const currentVersion = latestUpdated || versionResult.rows[0]?.version || `store-${storeId}-v0`;
